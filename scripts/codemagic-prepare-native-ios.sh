...
# Accept both naming conventions (VITE_* web prefix or plain names).
_supabase_url="${VITE_SUPABASE_URL:-${SUPABASE_URL:-}}"
_supabase_key="${VITE_SUPABASE_PUBLISHABLE_KEY:-${SUPABASE_ANON_KEY:-}}"

if [ -z "$_supabase_url" ]; then
  echo "ERROR: Missing SUPABASE_URL (or VITE_SUPABASE_URL)"; exit 1
fi
if [ -z "$_supabase_key" ]; then
  echo "ERROR: Missing SUPABASE_ANON_KEY (or VITE_SUPABASE_PUBLISHABLE_KEY)"; exit 1
fi
...
SUPABASE_URL_ESCAPED="$(escape_xcconfig_url "$_supabase_url")"
APP_WEB_HOST="${VITE_APP_WEB_HOST:-${APP_WEB_HOST:-app.frigy.app}}"
...
SUPABASE_ANON_KEY = ${_supabase_key}
