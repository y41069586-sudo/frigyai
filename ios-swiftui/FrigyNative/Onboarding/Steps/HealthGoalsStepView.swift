...
struct HealthGoalsStepView: View {
    ...
    private let options: [(id: String, icon: String, label: String)] = [
        ("more_energy",      "bolt.fill",          "Mehr Energie"),
        ("better_sleep",     "moon.fill",           "Besser schlafen"),
        ("less_stress",      "heart.fill",          "Weniger Stress"),
        ("immune_system",    "shield.fill",         "Immunsystem stärken"),
        ("muscle_building",  "dumbbell.fill",       "Muskelaufbau"),
        ("digestion",        "leaf.fill",           "Verdauung verbessern"),
        ("mental_clarity",   "brain.fill",          "Mentale Klarheit"),
        ("healthy_eating",   "fork.knife",          "Gesünder essen"),
    ]
        ...
        OnboardingStepScaffold(progress: progress, onBack: onBack) {
            ScrollView(showsIndicators: false) {
                VStack(spacing: 24) {
                    ...
                    OnboardingQuestion(text: "Was sind deine Gesundheitsziele?\n(Mehrfach auswählbar)")

                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                        ForEach(options, id: \.id) { opt in
                            let selected = draft.healthGoals.contains(opt.id)
                            Button {
                                if selected {
